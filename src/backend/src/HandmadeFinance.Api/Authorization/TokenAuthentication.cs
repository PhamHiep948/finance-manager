...
public sealed class TokenService(IConfiguration config) : ITokenIssuer
{
    private byte[] Key =>
        Encoding.UTF8.GetBytes(
            config["Authentication:SigningKey"]
                ?? "development-only-key-change-before-production-123456"
        );
    ...
    public string Issue(UserAccount user, DateTimeOffset expiresAt)
    {
        var payload = Convert.ToBase64String(
            Encoding.UTF8.GetBytes(
                JsonSerializer.Serialize(
                    new TokenData(user.Id, user.Role.ToString(), expiresAt.ToUnixTimeSeconds())
                )
            )
        );
        ...
        return payload
            + "."
            + Convert.ToBase64String(h.ComputeHash(Encoding.UTF8.GetBytes(payload)));
    }
        ...
        try
        {
            ...
            if (
                !CryptographicOperations.FixedTimeEquals(
                    h.ComputeHash(Encoding.UTF8.GetBytes(p[0])),
                    Convert.FromBase64String(p[1])
                )
            )
                return null;
            var d = JsonSerializer.Deserialize<TokenData>(
                Encoding.UTF8.GetString(Convert.FromBase64String(p[0]))
            );
            return d?.ExpiresAt > DateTimeOffset.UtcNow.ToUnixTimeSeconds() ? d : null;
        }
        catch
        {
            return null;
...
) : AuthenticationHandler<AuthenticationSchemeOptions>(options, logger, encoder)
{
    ...
    protected override Task<AuthenticateResult> HandleAuthenticateAsync()
    {
        ...
        var d = tokens.Validate(header[7..]);
        if (d is null)
