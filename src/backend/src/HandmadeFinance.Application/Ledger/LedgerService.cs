...
{
    Task<PageResult<LedgerEntry>> ListAsync(
        EntryKind kind,
        LedgerQuery query,
        CancellationToken ct
    );
    Task<LedgerEntry> GetAsync(EntryKind kind, long id, CancellationToken ct);
    Task<LedgerEntry> CreateAsync(
        EntryKind kind,
        LedgerWrite request,
        Actor actor,
        CancellationToken ct
    );
    Task<LedgerEntry> UpdateAsync(
        EntryKind kind,
        long id,
        LedgerWrite request,
        Actor actor,
...
public sealed class LedgerService(ILedgerRepository repository, IClock clock) : ILedgerService
{
    ...
    public async Task<LedgerEntry> GetAsync(EntryKind kind, long id, CancellationToken ct) =>
        await repository.GetAsync(kind, id, ct) is { DeletedAt: null } row
            ? row
            : throw AppException.NotFound(kind.ToString().ToLowerInvariant());

    public async Task<LedgerEntry> CreateAsync(
        EntryKind kind,
        LedgerWrite request,
        Actor actor,
        CancellationToken ct
    ...
    public async Task<LedgerEntry> UpdateAsync(
        EntryKind kind,
        long id,
        LedgerWrite request,
        Actor actor,
    ...
    )
    {
        RequireWrite(actor);
        Validate(request);
        var row = await GetAsync(kind, id, ct);
    ...
    {
        if (actor.Role is not (UserRole.ADMIN or UserRole.SHOP_OWNER))
            throw AppException.Forbidden();
        var row = await GetAsync(kind, id, ct);
    ...
    private static void ValidatePage(LedgerQuery q)
    {
        if (q.Page < 1 || q.PageSize is < 1 or > 100)
            throw AppException.Validation("page must be >= 1 and pageSize between 1 and 100.");
    }
    ...
    private static void Validate(LedgerWrite r)
    {
        ...
        if (expected != r.AmountAfterTax)
            throw AppException.Validation(
                "amountAfterTax is inconsistent with amount and taxPercent."
            );
    }
}
