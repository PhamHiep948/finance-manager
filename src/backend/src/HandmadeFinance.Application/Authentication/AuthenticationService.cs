...
public sealed class AuthenticationService(IUserRepository users, IPasswordService passwords, ITokenIssuer tokens, IClock clock)
{
    public async Task<LoginResult> LoginAsync(string email, string password, CancellationToken ct)
    {
        if (string.IsNullOrWhiteSpace(email) || string.IsNullOrEmpty(password)) throw AppException.Validation("Email and password are required.");
    ...
    internal static SafeUser Map(UserAccount u) => new(u.Id, u.Username, u.Email, u.FullName, u.Phone, u.Timezone, u.Role, u.IsActive);
}
