...
public sealed class ApiContractTests : IClassFixture<TestApiFactory>
{
    ...
    public async Task Login_returns_bearer_token_and_never_returns_password()
    {
        ...
        var response = await client.PostAsJsonAsync(
            "/api/v1/auth/login",
            new { email = "admin@handmade.local", password = "ChangeMe123!" }
        );
        Assert.Equal(HttpStatusCode.OK, response.StatusCode);
    ...
    public async Task Invalid_login_returns_safe_problem_details()
    {
        ...
        var response = await client.PostAsJsonAsync(
            "/api/v1/auth/login",
            new { email = "missing@example.com", password = "wrong" }
        );
        Assert.Equal(HttpStatusCode.Unauthorized, response.StatusCode);
    ...
    public async Task Dashboard_rejects_inverted_date_range_as_problem_details()
    {
        ...
        var response = await client.GetAsync(
            "/api/v1/dashboard?dateFrom=2026-09-18&dateTo=2026-09-17"
        );
        Assert.Equal(HttpStatusCode.BadRequest, response.StatusCode);
        Assert.Equal("application/problem+json", response.Content.Headers.ContentType?.MediaType);
    }
    ...
    public async Task Report_export_returns_documented_binary_format(
        string format,
        string mediaType,
        string signature
    )
    ...
    public async Task Report_export_rejects_unknown_format()
    {
        ...
        Assert.Equal(
            ...
            (await client.GetAsync("/api/v1/reports/export?format=CSV")).StatusCode
        );
    }
    ...
    public async Task Import_create_returns_202_and_can_be_read_from_history()
    {
        ...
        var id = batch.GetProperty("id").GetInt64();
        Assert.Equal(
            HttpStatusCode.OK,
            (await client.GetAsync($"/api/v1/imports/{id}")).StatusCode
        );
        var history = await client.GetFromJsonAsync<JsonElement>("/api/v1/imports");
        Assert.True(history.GetProperty("meta").GetProperty("totalItems").GetInt32() >= 1);
    }
    ...
    public async Task Attachment_upload_and_delete_flow_succeeds()
    {
        ...
        var id = entry.GetProperty("id").GetInt64();
        using var form = new MultipartFormDataContent();
        form.Add(new ByteArrayContent([1, 2, 3]), "file", "receipt.pdf");
        var uploaded = await client.PostAsync($"/api/v1/incomes/{id}/attachments", form);
        Assert.Equal(HttpStatusCode.Created, uploaded.StatusCode);
        var attachment = await uploaded.Content.ReadFromJsonAsync<JsonElement>();
        Assert.Equal(
            HttpStatusCode.NoContent,
            (
                await client.DeleteAsync(
                    $"/api/v1/attachments/{attachment.GetProperty("id").GetInt64()}"
                )
            ).StatusCode
        );
    }
    ...
    public async Task Attachment_rejects_missing_parent_and_empty_file()
    {
        using var client = await AuthenticatedClient();
        using var form = new MultipartFormDataContent();
        form.Add(new ByteArrayContent([1]), "file", "receipt.pdf");
        Assert.Equal(
            HttpStatusCode.NotFound,
            (await client.PostAsync("/api/v1/incomes/999999/attachments", form)).StatusCode
        );
    }
    ...
    {
        using var client = await AuthenticatedClient();
        await client.GetAsync("/api/v1/reports/export?format=PDF");
        var response = await client.GetFromJsonAsync<JsonElement>(
            "/api/v1/audit-logs?action=EXPORT&module=REPORT"
        );
    ...
    public async Task Report_export_rejects_every_unsupported_format(string format)
    {
        using var client = await AuthenticatedClient();
        var response = await client.GetAsync(
            $"/api/v1/reports/export?format={Uri.EscapeDataString(format)}"
        );
        Assert.Equal(HttpStatusCode.BadRequest, response.StatusCode);
    }
    ...
    [InlineData("2027-01-01", "2026-12-31")]
    [InlineData("9999-12-31", "0001-01-01")]
    public async Task Report_export_rejects_inverted_ranges(string from, string to)
    {
        using var client = await AuthenticatedClient();
        var response = await client.GetAsync(
            $"/api/v1/reports/export?format=PDF&dateFrom={from}&dateTo={to}"
        );
        Assert.Equal(HttpStatusCode.BadRequest, response.StatusCode);
    }
    ...
    {
        using var client = await AuthenticatedClient();
        using var form = Upload(fileName, [1]);
        Assert.Equal(
            HttpStatusCode.BadRequest,
            (await client.PostAsync("/api/v1/imports/preview", form)).StatusCode
        );
    }
    ...
    {
        using var client = await AuthenticatedClient();
        using var form = Upload("data.xlsx", [1], importType);
        Assert.Equal(
            HttpStatusCode.BadRequest,
            (await client.PostAsync("/api/v1/imports/preview", form)).StatusCode
        );
    ...
    public async Task Import_history_rejects_invalid_paging(int page, int pageSize)
    {
        ...
        Assert.Equal(
            HttpStatusCode.BadRequest,
            (await client.GetAsync($"/api/v1/imports?page={page}&pageSize={pageSize}")).StatusCode
        );
    }
    ...
    public async Task Import_detail_returns_404_for_missing_batch()
    {
        ...
        Assert.Equal(
            HttpStatusCode.NotFound,
            (await client.GetAsync("/api/v1/imports/9223372036854775807")).StatusCode
        );
    }
    ...
    {
        using var client = await AuthenticatedClient();
        using var form = Upload("empty.xlsx", []);
        Assert.Equal(
            HttpStatusCode.BadRequest,
            (await client.PostAsync("/api/v1/imports/preview", form)).StatusCode
        );
    ...
    public async Task Audit_rejects_invalid_paging(int page, int pageSize)
    {
        ...
        Assert.Equal(
            HttpStatusCode.BadRequest,
            (
                await client.GetAsync($"/api/v1/audit-logs?page={page}&pageSize={pageSize}")
            ).StatusCode
        );
    }
    ...
    public async Task Audit_rejects_inverted_date_range(string from, string to)
    {
        ...
        Assert.Equal(
            HttpStatusCode.BadRequest,
            (await client.GetAsync($"/api/v1/audit-logs?dateFrom={from}&dateTo={to}")).StatusCode
        );
    }
    ...
    public async Task Attachment_delete_returns_404_for_missing_id()
    {
        ...
        Assert.Equal(
            HttpStatusCode.NotFound,
            (await client.DeleteAsync("/api/v1/attachments/9223372036854775807")).StatusCode
        );
    }
    ...
    private static LedgerBody ValidLedger() =>
        new("2026-09-17", "Order", 1, 100m, 10m, 110m, "USD");

    private static MultipartFormDataContent Upload(
        string name,
        byte[] content,
        string importType = "INCOME"
    )
    {
