...
public sealed class LedgerBoundaryMatrixTests
{
    ...
    [InlineData(10.05, 5, 10.55)]
    [InlineData(99.99, 10, 109.99)]
    [InlineData(100, 20, 120)]
    [InlineData(123.45, 8, 133.33)]
    [InlineData(1000, 100, 2000)]
    [InlineData(999999.99, 0, 999999.99)]
    [InlineData(0.05, 10, 0.06)]
    [InlineData(0.10, 5, 0.11)]
    [InlineData(250.50, 12.5, 281.81)]
    public async Task Accepts_valid_monetary_and_tax_boundaries(
        double amount,
        double tax,
        double total
    )
    {
        ...
        var result = await f.Service.CreateAsync(
            EntryKind.INCOME,
            Write((decimal)amount, (decimal)tax, (decimal)total),
            Admin,
            default
        );
        Assert.Equal((decimal)total, result.AmountAfterTax);
    }

    [Theory]
    [InlineData(-1, 0, -1, "USD")]
    [InlineData(-0.01, 10, -0.01, "USD")]
    [InlineData(10, -1, 9.9, "USD")]
    [InlineData(10, 100.01, 20, "USD")]
    [InlineData(10, 101, 20.1, "USD")]
    [InlineData(10, 10, 10, "USD")]
    [InlineData(10, 10, 11.01, "USD")]
    [InlineData(10, 0, 10, "EUR")]
    [InlineData(10, 0, 10, "usd")]
    [InlineData(10, 0, 10, "")]
    [InlineData(0.01, 100, 0.01, "USD")]
    [InlineData(100, 5, 104.99, "USD")]
    public async Task Rejects_invalid_monetary_tax_or_currency_combinations(
        double amount,
        double tax,
        double total,
        string currency
    )
    {
        ...
        var error = await Assert.ThrowsAsync<AppException>(() =>
            f.Service.CreateAsync(
                EntryKind.INCOME,
                Write((decimal)amount, (decimal)tax, (decimal)total, currency),
                Admin,
                default
            )
        );
    ...
    {
        var f = new Fixture();
        var result = await f.Service.ListAsync(
            EntryKind.INCOME,
    ...
    public async Task Rejects_invalid_paging_boundaries(int page, int pageSize)
    {
        ...
        await Assert.ThrowsAsync<AppException>(() =>
            f.Service.ListAsync(EntryKind.INCOME, new(Page: page, PageSize: pageSize), default)
        );
    }
    ...
    {
        var f = new Fixture();
        await f.Add("Custom Order - đơn hàng 123");
        var result = await f.Service.ListAsync(EntryKind.INCOME, new(Search: search), default);
    ...
    [InlineData("2020-01-01", "2030-01-01")]
    ...
    {
        var f = new Fixture();
        await f.Add("row", new(2026, 1, 1));
        var result = await f.Service.ListAsync(
            EntryKind.INCOME,
            new(DateFrom: Parse(from), DateTo: Parse(to)),
            default
        );
        Assert.Single(result.Items);
    }
    ...
    public async Task Rejects_every_inverted_date_range(string from, string to)
    {
        ...
        await Assert.ThrowsAsync<AppException>(() =>
            f.Service.ListAsync(
                EntryKind.INCOME,
                new(DateFrom: DateOnly.Parse(from), DateTo: DateOnly.Parse(to)),
                default
            )
        );
    }
    ...
    [InlineData(UserRole.SHOP_OWNER, true)]
    ...
    public async Task Create_role_matrix_is_enforced(UserRole role, bool allowed)
    {
        var f = new Fixture();
        var action = () =>
            f.Service.CreateAsync(EntryKind.EXPENSE, Write(10, 0, 10), new Actor(5, role), default);
        if (allowed)
    ...
    private static LedgerWrite Write(
        decimal amount,
        decimal tax,
        decimal total,
        string currency = "USD"
    ) => new(new(2026, 1, 1), "row", 1, amount, tax, total, currency);
    ...
    private sealed class Fixture
    {
        ...
        public Task<LedgerEntry> Add(string description, DateOnly? date = null) =>
            Service.CreateAsync(
                EntryKind.INCOME,
                Write(10, 0, 10) with
                {
    ...
    private sealed class Repo : ILedgerRepository
    {
        ...
        public Task<LedgerEntry?> GetAsync(EntryKind k, long id, CancellationToken ct) =>
            Task.FromResult(rows.SingleOrDefault(x => x.Kind == k && x.Id == id));

        public Task<IReadOnlyList<LedgerEntry>> ListAsync(EntryKind k, CancellationToken ct) =>
