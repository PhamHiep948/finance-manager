...
public sealed class UserServiceTests
{
    ...
    private static UserWrite ValidWrite =>
        new(
            " new-user ",
            "new@example.com",
            " New User ",
    ...
    public async Task Only_admin_can_list_users(UserRole role)
    {
        ...
        var error = await Assert.ThrowsAsync<AppException>(() =>
            f.Service.ListAsync(new Actor(2, role), default)
        );
    ...
    public async Task Get_missing_user_returns_not_found()
    {
        var f = new Fixture();
        var error = await Assert.ThrowsAsync<AppException>(() =>
            f.Service.GetAsync(404, Admin, default)
        );
    ...
    public async Task Create_rejects_invalid_user_fields(
        string username,
        string email,
        string fullName
    )
    ...
    public async Task Create_rejects_password_outside_length_limits(string password)
    {
        ...
        await Assert.ThrowsAsync<AppException>(() =>
            f.Service.CreateAsync(ValidWrite, password, Admin, default)
        );
    }
    ...
    public async Task Create_rejects_duplicate_username_or_email()
    {
        var f = new Fixture();
        var error = await Assert.ThrowsAsync<AppException>(() =>
            f.Service.CreateAsync(ValidWrite with { Username = "alice" }, "secret", Admin, default)
        );
    ...
    {
        var f = new Fixture();
        var result = await f.Service.UpdateAsync(
            1,
            new(
                "alice2",
                "alice2@example.com",
    ...
    public async Task User_can_read_only_own_profile()
    {
        ...
        await Assert.ThrowsAsync<AppException>(() =>
            f.Service.ProfileAsync(new Actor(404, UserRole.EMPLOYEE), default)
        );
    }
    ...
    public async Task Profile_update_requires_name_and_timezone(string name, string timezone)
    {
        var f = new Fixture();
        var error = await Assert.ThrowsAsync<AppException>(() =>
            f.Service.UpdateProfileAsync(new(name, null, timezone), Employee, default)
        );
        Assert.Equal(400, error.Status);
    }
    ...
    public async Task Change_password_rejects_wrong_current_password()
    {
        var f = new Fixture();
        var error = await Assert.ThrowsAsync<AppException>(() =>
            f.Service.ChangePasswordAsync("wrong", "new-secret", Employee, default)
        );
    ...
    {
        public DateTimeOffset UtcNow => new(2026, 9, 17, 8, 30, 0, TimeSpan.Zero);
    }
    ...
    private sealed class FakeRepository : IUserRepository
    {
        ...
        public Task<IReadOnlyList<UserAccount>> ListAsync(CancellationToken ct) =>
            Task.FromResult<IReadOnlyList<UserAccount>>(Users.ToList());
